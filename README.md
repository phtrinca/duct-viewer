...
def admin_page(user=Depends(check_admin)):
    ...
    for i, d in enumerate(drawings):
        ...
        rows += (
            "<tr>"
            "<td style='padding:10px 12px;border-bottom:1px solid #334155;font-size:13px'>" + n + "</td>"
            "<td style='padding:10px 12px;border-bottom:1px solid #334155;font-size:13px'>" + str(c) + " ducts</td>"
            "<td style='padding:10px 12px;border-bottom:1px solid #334155'>"
            "<form method='post' action='/admin/delete/" + str(i) + "' style='display:inline'>"
            "<button style='background:#7f1d1d;color:#fca5a5;border:none;padding:5px 12px;border-radius:6px;cursor:pointer;font-size:12px'>Delete</button>"
            "</form>"
            "</td></tr>"
        )
    ...
    table = ""
    if drawings:
        table = "<table style='width:100%;border-collapse:collapse'><tr><th style='padding:10px 12px;text-align:left;font-size:11px;color:#64748b;border-bottom:1px solid #334155'>NAME</th><th style='padding:10px 12px;text-align:left;font-size:11px;color:#64748b;border-bottom:1px solid #334155'>DUCTS</th><th></th></tr>" + rows + "</table>"
    else:
        table = "<p style='color:#475569;font-size:13px;padding:16px 0'>No drawings uploaded yet.</p>"
...
.card h2{font-size:13px;font-weight:500;margin-bottom:16px;color:#94a3b8;letter-spacing:0.05em}
.upload-area{border:2px dashed #334155;border-radius:8px;padding:32px;text-align:center;cursor:pointer}
.upload-area:hover{border-color:#1D9E75}
.upload-area p{color:#64748b;font-size:14px;margin-top:8px}
input[type=file]{display:none}
input[type=text]{width:100%;padding:10px 14px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#f1f5f9;font-size:14px;outline:none;margin-bottom:12px;margin-top:12px}
input[type=text]:focus{border-color:#1D9E75}
.btn{padding:10px 20px;border-radius:8px;border:none;cursor:pointer;font-size:14px;font-weight:600;width:100%;background:#1D9E75;color:#fff}
...
<div class='container'>
  <div class='card'>
    ...
    <button class='btn' onclick='upload()'>Process and Upload</button>
    <div class='progress' id='progress'><div class='progress-bar' id='bar'></div></div>
    <div id='status'></div>
  </div>
  <div class='card'>
    <h2>DRAWINGS (""" + str(len(drawings)) + """ total)</h2>
    """ + table + """
  </div>
...
async function upload() {
  ...
  try {
    ...
    if (res.ok) {
      status.textContent = 'Done! ' + data.count + ' ducts extracted from ' + name;
